...
vim.keymap.set({ 'n', 'v' }, '<leader>sw', telescope_builtin.grep_string, { desc = '[S]earch current [W]ord' })
vim.keymap.set('n', '<leader>fw', telescope_builtin.live_grep, { desc = '[S]earch by [G]rep' })
...
vim.keymap.set(
  'n',
  '<leader>sn',
  function() telescope_builtin.find_files { cwd = vim.fn.stdpath 'config', follow = true } end,
